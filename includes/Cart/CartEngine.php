...
final class CartEngine {
	...
	public function init( Loader $loader ): void {
		...
		// Zero out gift item prices in the cart session so WC totals are correct.
		$loader->add_action( 'woocommerce_before_calculate_totals',     [ $this, 'zero_gift_prices' ], 5 );

		// Allow gifts to pass WooCommerce's purchasable/stock checks at checkout.
		$loader->add_filter( 'woocommerce_cart_item_is_purchasable',    [ $this, 'gift_is_purchasable' ], 10, 3 );
		$loader->add_filter( 'woocommerce_cart_item_product',           [ $this, 'gift_cart_item_product' ], 10, 3 );
	...
	/**
	 * Sets gift item prices to zero in the cart session so WooCommerce
	 * calculates totals correctly.
	 *
	 * @param \WC_Cart $cart
	 */
	public function zero_gift_prices( \WC_Cart $cart ): void {
		if ( is_admin() && ! defined( 'DOING_AJAX' ) ) {
			return;
		}
	...
	/**
	 * Allows gift cart items to pass WooCommerce's purchasable check.
	 * Without this, WC may block checkout if a gift product is set to
	 * "sold individually" or has stock quirks.
	 *
	...
	/**
	 * Ensures the gift product object has its price zeroed for stock/validation
	 * checks that operate on the product object from the cart item.
	 *
	 * @param \WC_Product         $product
	 * @param array<string,mixed> $cart_item
